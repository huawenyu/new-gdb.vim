...
let s:gdb_local_vert = {
        ...
        \      'control': "client",
        \      'h_sz': "%12",
        \      'init': ['termopen', s:const_bash],
        \  }
        \}

let s:gdb_local_horiz = {
        \  'name': 'gdb_local_horiz',
        \  'set_all': ['nonu', 'nornu'],
        \  'right': {
        \      'viewname': "client",
        \      'control': "client",
...
let s:gdb_remote_vert = {
        \  'name': 'gdb_remote_vert',
        \  'set_all': ['nonu', 'nornu'],
        \  'bot': {
        \      'viewname': "server",
        \      'control': "server",
...
let s:gdb_remote_horiz = {
        \  'name': 'gdb_remote_horiz',
        \  'set_all': ['nonu', 'nornu'],
        \  'right': {
        \      'viewname': "server",
        \      'control': "server",
...
let s:gdb_control = {
        ...
        \              "next":    "conn_succ",
        \          },
        \          {   "match":   [ '\v^Remote debugging using \w+:\d+', ],
        \              "hint":    "gdb.RemoteConnectSucc",
        \              "action":  "",
        \              "next":    "conn_succ",
        \          },
        \          {   "match":   [ '\v^\d+\.\d+\.\d+\.\d+:\d+: Connection refused.', ],
        ...
        \              "next":    "pause",
        \          },
        \          {   "match":   [ '\v^\d+\.\d+\.\d+\.\d+:\d+:\d+: Connection timed out.', ],
        \              "hint":    "gdb.RemoteConnectTimeout",
        \              "test":    "dut:444: Connection timed out.",
        \              "action":  "",
        \              "next":    "pause",
        \          },
        \          {   "match":   [ '\v^\w+:\d+: Connection timed out.', ],
        \              "hint":    "gdb.RemoteConnectTimeout",
        \              "test":    "dut:444: Connection timed out.",
        \              "action":  "",
        \              "next":    "pause",
        ...
        \              "next":    "pause",
        \          },
        \          {   "match":   [ '\v^neobugger_local_start', ],
        \              "comment": "mode=Local: already start now",
        \              "hint":    "gdb.LocalStart",
        \              "test":    "neobugger_local_start",
        \              "action":  "",
        \              "next":    "pause",
        \          },
        \      ],
        ...
        \              "comment": "gdb pause and show code line",
        \              "hint":    "gdb.Jump",
        \              "test":    "$3 = (int *) 0x7fffffffd93c",
        \              "action":  "on_print",
        \              "next":    "pause",
        \          },
        \          {   "match":   ['Remote connection closed.'],
        \              "hint":    "gdb-server closed",
        \              "action":  "on_disconnected",
        \              "next":    "target",
        \          },
        ...
        \      "init": [
        \          {   "match":   [ '^newRuntime ', ],
        \              "hint":    "Gdbserver waiting client start succ.",
        \              "action":  "",
        \              "event":   "init",
        \              "next":    "listen",
        \          },
        ...
        \          {   "match":   [ '\v^Remote debugging using \d+\.\d+\.\d+\.\d+:\d+', ],
        \              "hint":    "gdb.RemoteConnectSucc",
        \              "action":  "on_remoteconn_succ",
        \              "next":    "connecting",
        \          },
        \          {   "match":   [ '\v^Remote debugging from host \d+\.\d+\.\d+\.\d+:\d+', ],
        \              "hint":    "gdb.RemoteConnectSucc",
        \              "action":  "on_remoteconn_succ",
        \              "next":    "connecting",
        \          },
...
call NewAddLayout(s:gdb_remote_vert, s:gdb_control, s:prototype)
call NewAddLayout(s:gdb_remote_horiz, s:gdb_control, s:prototype)
...
function! s:prototype.Send(data)
    ...
    call self._Send(state, a:data, 0)
endfunction
...
function! s:prototype.Watch(expr)
    let l:__func__ = "new.vim:gdb:watch() "
    let expr = a:expr
    if expr[0] != '&'
        let expr = '&' . expr
    endif
...
function! s:prototype.on_open(model, state, match_list) abort
    let l:__func__ = "new.vim:gdb:on_open() "
...
function! s:prototype.on_start(model, state, match_list) abort
    let l:__func__ = "new.vim:gdb:on_start() "
    "silent! call s:log.debug(l:__func__, "model=", a:model, " state=", a:state, " matched=", a:match_list)
...
function! s:prototype.on_print(model, state, match_list) abort
    let l:__func__ = "new.vim:gdb:on_print() "
    "silent! call s:log.debug(l:__func__, "model=", a:model, " state=", a:state, " matched=", a:match_list)
...
function! s:prototype.on_server_accept(model, state, match_list) abort
    let l:__func__ = "new.vim:gdb:on_server_accept() "
    "silent! call s:log.debug(l:__func__, "model=", a:model, " state=", a:state, " matched=", a:match_list)
...
function! s:prototype.Map(viewname, type)
    let l:__func__ = "gdb.Map() "
    call new#util#MarkActiveWindow()

    let l:win_id = 0
    if empty(a:viewname)
        let l:win_id = g:vmwRuntime.wid_main
    elseif has_key(g:vmwRuntime.models, a:viewname)
        let l:model = g:vmwRuntime.models[a:viewname]
        let l:win_id = l:model._win_id
    else
        silent! call s:log.debug(l:__func__, "failed: viewname=", a:viewname, " type=", a:type)
        return
    endif

    if win_gotoid(l:win_id) != 1
        return
    endif
    ...
    call new#util#RestoreActiveWindow()
endfunction
...
command! -nargs=0 GdbContinue call s:this.Send('c')
...
command! -nargs=0 GdbUntil call s:this.TBreak()
